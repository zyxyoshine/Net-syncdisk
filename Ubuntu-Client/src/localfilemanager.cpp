...
void TMY::LocalFileWriter::setFile(const FilePath filepath_, const int len_)
{
   ...
   std::ofstream metafile(metaPath.toStdString());
    ...
    log("Writer: get fulllen = " + QString::number(fulllen));
    metaPath = path;
    metaPath += METATAIL;
...
TMY::PullReq TMY::LocalFileWriter::generatePullReqEntry()
{
    ...
    for (int i = 0; i < chunks.size(); ++i)
    {
        ...
        {
            p.len = tail - p.offset;
            log("Writer: Pull.offset " + QString::number(p.offset));
            pullReq.push_back(p);
        }
        tail += chunks[i].len;
        p.offset = tail;
...
void TMY::LocalFileWriter::writePush(PushReqEntry &pushReq)
{
    ...
    log("Writer: Push.offset " + QString::number(pushReq.offset));
    log("Writer: Push.len    " + QString::number(pushReq.len));
